...
def to_h1md(result: ScanResult) -> str:
    ...
    lines.append(
        ...
        f"{len(findings)} finding(s).{elapsed_str}_"
    )
    lines.append("")

    # Severity breakdown — only shown when there are findings so readers see the
    # risk profile at a glance without scanning every finding header.
    if findings and result.summary.by_severity:
        _SEV_ORDER = ["critical", "high", "medium", "low", "info"]
        sev_parts = [
            f"{sev}: {result.summary.by_severity[sev]}"
            for sev in _SEV_ORDER
            if sev in result.summary.by_severity
        ]
